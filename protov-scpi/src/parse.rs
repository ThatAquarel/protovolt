use crate::block;
use crate::command::{
    ChannelParam, FWUP_DATA_PREFIX, FWUP_SIGNATURE_LEN, MeasKind, ScpiCommand, TempSlot,
};
use crate::types::{LINE_BUF, RegisterChannel, Rgb, ScpiChannel};

pub fn normalize_command(raw: &str) -> heapless::String<LINE_BUF> {
    let trimmed = raw.trim();
    let mut out = heapless::String::<LINE_BUF>::new();
...
fn parse_fwup(s: &str) -> Option<ScpiCommand> {
    ...
    if let Some(hex) = s.strip_prefix("SYST:FWUP:APPL ") {
        let bytes = block::decode_hex_block(hex.trim())?;
        if bytes.len() != FWUP_SIGNATURE_LEN {
            return None;
        }
...
fn parse_color_set(s: &str) -> Option<(ScpiChannel, Rgb)> {
    ...
    crate::types::parse_rgb_triplet(parts[1])
        .ok()
        .map(|rgb| (ch, rgb))
}
...
fn parse_brightness_set(s: &str, prefix: &str) -> Option<u8> {
    ...
    crate::types::parse_brightness(rest).ok()
}
